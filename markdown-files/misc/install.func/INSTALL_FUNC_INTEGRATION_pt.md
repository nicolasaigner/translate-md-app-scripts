# Guia de Integração do install.func

Como o install.func se integra ao ecossistema ProxmoxVE e se conecta a outras bibliotecas de funções.

## Integração de Componentes

### install.func no Pipeline de Instalação

```
install/app-install.sh (container-side)
    │
    ├─ Sources: core.func (messaging)
    ├─ Sources: error_handler.func (error handling)
    │
    ├─ ★ Uses: install.func ★
    │  ├─ setting_up_container()
    │  ├─ network_check()
    │  ├─ update_os()
    │  └─ motd_ssh()
    │
    ├─ Uses: tools.func (package installation)
    │
    └─ Back to install.func:
       ├─ customize()
       └─ cleanup_lxc()
```

### Integração com tools.func

install.func e tools.func funcionam em conjunto:

```
setting_up_container()          [install.func]
    │
update_os()                     [install.func]
    │
pkg_update()                    [tools.func]
setup_nodejs()                  [tools.func]
setup_mariadb()                 [tools.func]
    │
motd_ssh()                      [install.func]
customize()                     [install.func]
cleanup_lxc()                   [install.func]
```

---

## Dependências

### Dependências Externas

- `curl`, `wget` - Para downloads
- `apt-get` ou `apk` - Gerenciamento de pacotes
- `ping` - Verificação de rede
- `systemctl` ou `rc-service` - Gerenciamento de serviços

### Dependências Internas

```
install.func uses:
├─ core.func (for messaging and colors)
├─ error_handler.func (for error handling)
└─ tools.func (for package operations)
```

---

## Boas Práticas

### Sempre Siga Este Padrão

```bash
#!/usr/bin/env bash
source /dev/stdin <<<"$FUNCTIONS_FILE_PATH"

# 1. Setup error handling
catch_errors

# 2. Initialize container
setting_up_container

# 3. Verify network
network_check

# 4. Update OS
update_os

# 5. Installation (your code)
# ... install application ...

# 6. Configure access
motd_ssh

# 7. Customize
customize

# 8. Cleanup
cleanup_lxc
```

---

**Última Atualização**: Dezembro de 2025
**Mantenedores**: Equipe community-scripts
